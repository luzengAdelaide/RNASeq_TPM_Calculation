...
trim_galore --stringency 6 -o $i/trim/ --paired -fastqc_args "-t 8" ${READPATH}/$i/${FILES3[$SLURM_ARRAY_TASK_ID]}_1.fastq.gz ${READPATH}/$i/${FILES3[$SLURM_ARRAY_TASK_ID]}_2.fastq.gz
...
trim_galore --clip_R1 5 --three_prime_clip_R1 5 -o $i/trim/ -fastqc_args "-t 8" ${READPATH}/${FILES2[$SLURM_ARRAY_TASK_ID]}_1.fastq.gz
...
# Pair-end
~/RSEM-1.3.0/rsem-calculate-expression -p 8 --bowtie2 --paired-end ${READPATH}/$i/trim/${FILES3[$SLURM_ARRAY_TASK_ID]}_1_val_1.fq.gz ${READPATH}/$i/trim/${FILES3[$SLURM_ARRAY_TASK_ID]}_2_val_2.fq.gz $i_index/$iGenome ${READPATH}/$i/trim/${FILES3[$SLURM_ARRAY_TASK_ID]}
# Single-end
~/RSEM-1.3.0/rsem-calculate-expression -p 8 --bowtie2 ${READPATH}/$i/trim/${FILES2[$SLURM_ARRAY_TASK_ID]}_1_trimmed.fq.gz $i_index/$iGenome ${READPATH}/$i/trim/${FILES2[$SLURM_ARRAY_TASK_ID]}
